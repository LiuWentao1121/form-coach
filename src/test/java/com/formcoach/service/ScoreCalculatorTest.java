...
class ScoreCalculatorTest {
    ...
    void oneErrorShouldDeduct() {
        List<ErrorInfo> errors = List.of(
                ErrorInfo.builder().joint("左膝").angle(75.0).expected("75-100").tip("再蹲低").build()
        );
        int score = calculator.calculateFrameScore(errors);
        assertTrue(score < 100, "Score should be less than 100 with errors");
        assertTrue(score >= 75, "One error should not drop below 75, got: " + score);
    }

    @Test
    void coreErrorShouldDeductMore() {
        List<ErrorInfo> errors = List.of(
                ErrorInfo.builder().joint("核心").angle(150.0).expected("170-175").tip("收紧核心").build()
        );
    ...
    void scoreShouldNotGoBelowZero() {
        List<ErrorInfo> errors = List.of(
                ErrorInfo.builder().joint("核心").angle(0.0).expected("170-175").tip("e1").build(),
                ErrorInfo.builder().joint("腰背").angle(0.0).expected("60-90").tip("e2").build(),
                ErrorInfo.builder().joint("左膝").angle(0.0).expected("75-100").tip("e3").build(),
                ErrorInfo.builder().joint("右膝").angle(0.0).expected("75-100").tip("e4").build(),
    ...
    void shouldCalculateSessionScore() {
        List<Integer> frameScores = List.of(100, 90, 75, 95, 85);
